...
class AnimeInfoRepository {
    ...
    fun fetchEpisodeList(id: String, startEpisode:String, endEpisode: String, alias: String): Observable<ResponseBody> {
        val animeEpisodeService = retrofit.create(NetworkInterface.FetchEpisodeList::class.java)
        return animeEpisodeService.get(id = id, startEpisode = startEpisode, endEpisode = endEpisode, alias = alias, header = Utils.getHeader())
